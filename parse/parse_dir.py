...
import os
import sys
from xactimate_rough_draft_parse import XactimateRoughDraftParser
...
def main():
    ...
    for a in sys.argv[1:]:
        ...
        if a.startswith("--out="): out_arg = a.split("=", 1)[1].strip()
    ...
    if os.path.isdir(in_arg):
        ...
        for pdf in sorted(pdfs):
            parser = XactimateRoughDraftParser(input_file=os.path.join(in_arg, pdf),
                                               output_path=out_arg)  # directory => auto-stem
            parser.run()
    else:
        # single file
        if not os.path.exists(in_arg):
            ...
            return
        parser = XactimateRoughDraftParser(input_file=in_arg, output_path=out_arg)
        parser.run()
