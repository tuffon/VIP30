...
must_haves:
  truths:
    - "pytest packages/parser/tests/test_coverage.py -v runs all 12 tests to completion without crashing"
    - "Rough-draft tests (lachman, kalyvas) pass — 100% section coverage"
    - "Final-draft tests fail with informative field-level diffs (section name, item count, dollar delta)"
    - "packages/parser/tests/GAP-REPORT.md exists and documents per-doc coverage% + cross-doc gap summary"
  artifacts:
    - path: "packages/parser/scripts/generate_gap_report.py"
      provides: "Self-contained script that runs all 6 golden master comparisons and writes GAP-REPORT.md"
      min_lines: 100
    - path: "packages/parser/tests/GAP-REPORT.md"
      provides: "v2.5 gap analysis — per-doc coverage%, missing sections, metadata gaps, cross-doc pattern summary"
      min_lines: 80
  key_links:
    - from: "scripts/generate_gap_report.py"
      to: "vip_parser.xactimate.XactimateRoughDraftParser"
      via: "imports and invokes parser in tempdir"
      pattern: "XactimateRoughDraftParser"
    - from: "scripts/generate_gap_report.py"
      to: "tests/GAP-REPORT.md"
      via: "writes report to that path"
      pattern: "GAP-REPORT"
---

<objective>
Run the Phase 25 coverage harness and produce the structured gap report that feeds v2.5 planning.

Purpose: Execute the coverage tests against all 6 golden masters to confirm rough-draft baseline integrity and capture the full final-draft gap inventory. Write GAP-REPORT.md as the direct v2.5 parser-fix input.
Output: generate_gap_report.py (reproducible report generator), GAP-REPORT.md (gap inventory).
</objective>
...
<task type="auto">
  <name>Task 1: Create generate_gap_report.py</name>
  <files>packages/parser/scripts/generate_gap_report.py</files>
  <action>
Create packages/parser/scripts/generate_gap_report.py — a self-contained script that runs all 6
golden master comparisons and writes packages/parser/tests/GAP-REPORT.md.

The script duplicates the run_parser/comparison helpers from conftest.py to stay self-contained
(avoids complex test-module imports; the helpers are ~30 lines each).
...
```
  ...
  <verify>
    cd packages/parser && python scripts/generate_gap_report.py 2>&1 | head -40
    Should print per-document coverage lines and "Wrote: packages/parser/tests/GAP-REPORT.md".
    Check: wc -l tests/GAP-REPORT.md (expect ≥80 lines)
  </verify>
  <done>
    packages/parser/scripts/generate_gap_report.py exists and runs without import error.
    packages/parser/tests/GAP-REPORT.md written with ≥80 lines.
    Script output shows per-document coverage% for all 6 docs.
    Rough-draft docs show 100% (or explicit REGRESSION DETECTED if not).
  </done>
</task>

<task type="auto">
  <name>Task 2: Run pytest harness and verify expected results</name>
  <files>(no new files — verification only)</files>
  <action>
Run the full coverage harness and confirm expected pass/fail semantics:

```
cd packages/parser && pytest tests/test_coverage.py -v 2>&1
```

Expected results:
- lachman rough-draft: PASS (both test_metadata and test_section_coverage)
- kalyvas rough-draft: PASS (both test_metadata and test_section_coverage)
- bschacter contractor-final: FAIL on test_section_coverage (0 sections parsed — known v2.5 gap)
- sf_bschacter statefarm: FAIL on test_section_coverage (partial coverage — known v2.5 gap)
- lachman_sf statefarm: FAIL on test_section_coverage (partial coverage — known v2.5 gap)
- kalyvas_sf statefarm: FAIL on test_section_coverage (partial coverage — known v2.5 gap)

If a rough-draft test FAILS, that is a REGRESSION — investigate and fix before marking plan done.

If final-draft tests PASS unexpectedly (parser somehow produces correct output), update
GAP-REPORT.md to reflect this as a positive finding.

If any test crashes with an import error or uncaught exception (not a pytest.fail()), fix the
test or conftest issue before proceeding.

Accept: rough-draft tests pass + final-draft tests fail with informative output (not crashes).
  </action>
  <verify>
    cd packages/parser && pytest tests/test_coverage.py -v 2>&1 | tail -20
    Look for:
    - "2 passed" for rough-draft documents
    - FAILED lines for final-draft documents (not ERROR lines)
    - No "ImportError" or "ModuleNotFoundError" in output
  </verify>
  <done>
    pytest output shows lachman and kalyvas rough-draft tests passing.
    Final-draft failures are pytest.fail() assertions (not crashes/errors).
    Failure messages contain document name and section-level diff information.
  </done>
</task>
...
- [ ] python packages/parser/scripts/generate_gap_report.py runs to completion
- [ ] packages/parser/tests/GAP-REPORT.md exists with ≥80 lines
- [ ] GAP-REPORT.md has Summary table, Per-Document Analysis, Cross-Document Patterns sections
- [ ] pytest tests/test_coverage.py -v shows lachman + kalyvas passing
- [ ] pytest final-draft failures show field-level diffs, not crashes
- [ ] GAP-REPORT.md shows rough-draft baseline: PASS
</verification>

<success_criteria>
- generate_gap_report.py is self-contained and reproducible
- GAP-REPORT.md documents the complete v2.5 gap inventory
- Rough-draft baseline confirmed (100% coverage)
- Final-draft gaps documented by document type, section name, and item/dollar delta
- Cross-doc patterns identified for v2.5 prioritization
</success_criteria>

<output>
After completion, create `.planning/phases/25-coverage-harness/25-02-SUMMARY.md`
</output>
