# Requirements Archive: v2.6 Pipeline Rewrite
...
- [x] **TRADE-01**: Pipeline builds `TradeContext` from `recap_by_category` in both estimate parser JSONs — normalized category totals available for all 6 doc types
- [x] **TRADE-02**: When `trade_summary` field is present (StateFarm final-draft format), enrich `TradeContext` with trade summary data
- [x] **TRADE-03**: When `recap_by_category` is absent, synthesize category totals from section-level line items as last-resort fallback

### Cost Driver Identification

- [x] **DRIVER-01**: Pipeline identifies top cost drivers by absolute dollar delta across categories, sorted descending — deterministic, no LLM
- [x] **DRIVER-02**: Pipeline maps all line items from each top driver category in both estimates to a `DriverWithItems` model
- [x] **DRIVER-03**: Pipeline verifies that mapped line items sum approximately to category total before passing to LLM — logs discrepancy note if verification fails
...
- **PERF-01**: Async parallel per-driver LLM calls — sync is acceptable for v2.6; parallelize only if profiling identifies bottleneck
  *Status: Deferred to v2.7+ — sync acceptable, no profiling bottleneck identified*
...
---
*Archived: 2026-03-16 as part of v2.6 milestone completion*
